...
from utils.prompt_formatter import format_prompt_infer_seq
...
def main(args):
    ...
    for alpha in alpha_list:
        for theta in theta_list:
            prompt = format_prompt_infer_seq(args.prompt_path, args.initial_budget, refer_ratios, alpha, rho, theta, T=args.total_step)
            for seed in seed_list:
                ...
                res["output"] = output
                res["valid"] = (pred_ratios is not None)
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        '--reparse_answer', action=argparse.BooleanOptionalAction, default=False
    )
    parser.add_argument(
